"""Authored mutants for nucleus/check.sh — run by nucleus/mutation_probe.py.
    ...
    venv/bin/python nucleus/mutation_probe.py tests/mutants_check_coverage.py
...
MUTANTS = {
    ...
    # 08-13 with a probe oracle: a committed tests/test_*.py that check.sh never runs,
    ...
    "M3 invocation degraded to a bare prose mention":
        (_CHARTER, 'echo "see tests/test_charter.py for the charter invariants"'),
    ...
    "M5 check.sh invokes a deleted oracle":
        (_CHARTER, 'run "ghost gate" "$PY" tests/test_this_was_deleted.py'),
