...
for a in $AGENTS; do
  ...
  tmux split-window -t wall -d \
    "while :; do tmux attach -r -t ax-$a 2>/dev/null; clear; echo '  [$a is down — nucleus can respawn it]'; sleep 5; done"
  new=$(tmux list-panes -t wall -F '#{pane_id}' | tail -1)
  tmux select-pane -t "$new" -T "$a"
