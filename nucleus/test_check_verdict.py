...
"""Oracle for nucleus/check.sh's SKIP accounting — the verdict may never out-claim its parts.
    ...
    venv/bin/python nucleus/test_check_verdict.py      (also run by nucleus/check.sh)
...
(cf. nucleus/test_card_canon.py, where the card's own signer was its own verifier).
